...
class strLabelConverter(object):
    ...
    def encode(self, texts):
        ...
        list = config.alphabet_v2
        label = np.zeros(((len(texts),config.nclass))).astype('float32')
        for i in range(len(texts)):
            for ln in texts[i]:
                if ln != ' ':
                    ln = ord(ln if self._ignore_case else ln)
                    index = list.index(ln)
                    label[i][int(index)] += 1  # label construction for ACE
    ...
    def decode(self, t, length, raw=False):
        ...
        """
        if length == 1:
                return ''.join(chr(self.alphabet[t]))
        else:
            # batch mode
